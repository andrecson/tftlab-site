...
/** A comp's tier-list badge: a single item OR augment icon (or neither). */
export interface CompBadgeInput {
  itemId: string | null;
  augmentId: string | null;
}

/**
 * Set a comp's tier-list badge to one item OR augment icon (at most one is
 * stored). Independent of tier/flags — ANY comp can carry a badge. Clearing both
 * removes it. Item wins if both arrive.
 */
export async function updateCompBadge(
  compId: string,
  input: CompBadgeInput,
): Promise<CompActionResult> {
  await requireRole("EDITOR");
  ...
  await db.comp.update({
    where: { id: compId },
    data: { badgeItemId: itemId, badgeAugmentId: augmentId },
  });
